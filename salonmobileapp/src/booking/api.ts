...
export async function fetchStaff(
  config: MobileApiConfig,
  tenantSlug: string,
  options?: { serviceIds?: number[] }
): Promise<BookingStaff[]> {
  const qs = new URLSearchParams();
  options?.serviceIds?.forEach((id, i) => qs.set(`service_ids[${i}]`, String(id)));
  const suffix = qs.toString() ? `?${qs}` : "";
  const res = await createApiClient({ ...config, tenantSlug }).get<{ data: BookingStaff[] }>(
    `${bookingBase(tenantSlug)}/staff${suffix}`
  );
  return Array.isArray(res.data) ? res.data : [];
}
...
export async function joinWaitlist(
  ...
  const res = await createApiClient({ ...config, tenantSlug }).post<{
    data: { uuid: string; status: string };
    message?: string;
  }>(`${bookingBase(tenantSlug)}/waitlist`, body);
  return res.data;
}
