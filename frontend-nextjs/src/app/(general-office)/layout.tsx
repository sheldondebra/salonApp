...
export default function GeneralOfficeLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <GeneralOfficeLayoutGate>{children}</GeneralOfficeLayoutGate>
    </Suspense>
  );
}

function GeneralOfficeLayoutGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (sessionExpired) {
      router.replace("/login?next=/general-office/payment-gateways");
    }
